...
function wc_crm_get_num_orders($val='', $field = '_customer_user', $string = false){
	...
	$num_orders_status = get_option('woocommerce_crm_total_value');
...
{
	global $wpdb;
		$sql    = "SELECT ID, post_title FROM {$wpdb->posts} WHERE post_type = 'wc_crm_accounts' AND post_status != 'trash' AND post_status != 'auto-draft' ";
		$result = $wpdb->get_results($sql);
	if($option && $result){
		$ac = array();
...
function wc_crm_get_customer_account($email)
{
	global $wpdb;
		$sql    = "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = '_wc_crm_customer_email' AND meta_value = '{$email}' LIMIT 1";
...
function wc_crm_get_lead_source()
{
	return apply_filters('wc_crm_lead_source', array(
		'advertisement'     => __('Advertisement', 'wc_customer_relationship_manager'),
		'cold_call'         => __('Cold Call', 'wc_customer_relationship_manager'),
		'employee_referral' => __('Employee Referral', 'wc_customer_relationship_manager'),
		'external_referral' => __('External Referral', 'wc_customer_relationship_manager'),
		'online_store'      => __('Online Store', 'wc_customer_relationship_manager'),
		'partner'           => __('Partner', 'wc_customer_relationship_manager'),
		'public_relations'  => __('Public Relations', 'wc_customer_relationship_manager'),
		'sales_mail_alias'  => __('Sales Mail Alias', 'wc_customer_relationship_manager'),
		'seminar_partner'   => __('Seminar Partner', 'wc_customer_relationship_manager'),
		'seminar_internal'  => __('Seminar Internal', 'wc_customer_relationship_manager'),
		'trade_show'        => __('Trade Show', 'wc_customer_relationship_manager'),
		'web_download'      => __('Web Download', 'wc_customer_relationship_manager'),
		'web_research'      => __('Web Research', 'wc_customer_relationship_manager'),
		'web_cases'         => __('Web Cases', 'wc_customer_relationship_manager'),
		'web_mail'          => __('Web Mail', 'wc_customer_relationship_manager'),
		'chat'              => __('Chat', 'wc_customer_relationship_manager'),
		));
}
function wc_crm_get_lead_status()
{
	return apply_filters('wc_crm_lead_status', array(
		'attempted_to_contact' => __('Attempted to Contact', 'wc_customer_relationship_manager'),
		'contact_in_future'    => __('Contact in Future', 'wc_customer_relationship_manager'),
		'contacted'            => __('Contacted', 'wc_customer_relationship_manager'),
		'junk_lead'            => __('Junk Lead', 'wc_customer_relationship_manager'),
		'lost_lead'            => __('Lost Lead', 'wc_customer_relationship_manager'),
		'not_contacted'        => __('Not Contacted', 'wc_customer_relationship_manager'),
		'pre_qualified'        => __('Pre Qualified', 'wc_customer_relationship_manager'),
		));
}
...
{
	$new_actions = array();
	$aenc = get_option('woocommerce_crm_automatic_emails_new_customer');
	if($aenc == '') $aenc = 'yes';
...
function wcrm_customer_bought_products( $customer_email, $user_id) {
	...
	$sql = $wpdb->prepare( "SELECT itemmeta.meta_value as ID, COUNT( itemmeta.meta_value ) as items_count, SUM(line_total.meta_value) as line_total
			...
			WHERE
				posts.post_status IN ( {$orders_statuses} )
