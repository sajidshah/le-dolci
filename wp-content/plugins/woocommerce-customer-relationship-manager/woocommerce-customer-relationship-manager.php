...
final class WooCommerce_Customer_Relationship_Manager {
   ...
   */
    ...
    public function __wakeup() {
      ...
      add_action('init', array($this, 'allow_large_joins'));
      
		}
    function allow_large_joins()
    {
        global $wpdb;
    ...
    public function woocommerce_available_payment_gateways($available_gateways)
    {
      ...
      $payment_method = get_user_meta( $user_id, 'preferred_payment_method' , true );

      $_available_gateways = array();
      foreach ( $available_gateways as $id => $gateway ) {
        $_available_gateways[$id] = $gateway;
        if( $id == $payment_method )
          $_available_gateways[$id]->chosen = true;
      }
  ...
  private function define_constants() {
        ...
        ?>
          ...
          //$formatted_billing_address = str_replace('<br />', '<br />\\', $formatted_billing_address);
            foreach ($__b_address as $key => $field ) { ?>
             jQuery('#_billing_<?php echo $key; ?>').val( "<?php echo addslashes(get_user_meta( $user_id, 'billing_'.$key, true ) ) ;?>" );
          <?php
            }
            foreach ($__s_address as $key => $field ) { ?>
             jQuery('#_shipping_<?php echo $key; ?>').val( "<?php echo addslashes(get_user_meta( $user_id, 'shipping_'.$key, true ) ) ;?>" );
          <?php
            }
          ?>
    ...
    public function deactivate(){      
      ...
      if ( is_admin() && !is_network_admin() ) {
        ...
        require_once( 'admin/classes/wc_crm_importer.php' );
    ...
    public function woocommerce_crm_restrict_list_logs() {
        $woocommerce_crm_filters_log = array(
          ...
          if(isset($_GET['log_status']) && $_GET['log_status'] == 'trash'){ ?>
            <input type="submit" value="<?php echo _e('Empty Trash', 'wc_customer_relationship_manager'); ?>" class="button apply" id="delete_all" name="delete_all">
            <?php
          }
          ?>
        </div>
