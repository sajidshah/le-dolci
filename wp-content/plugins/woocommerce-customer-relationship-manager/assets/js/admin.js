...
jQuery('document').ready(function($){
    ...
    if( $('.chosen_select').length > 0 ){
        if( $().select2 ){
            $('select.chosen_select').css('min-width', '200px').select2();
        }else{
            $('select.chosen_select').css('min-width', '200px').chosen({search_contains:true});            
        }
    }
    ...
    if( $().select2 ){
        $('select#group_customer_status, select#group_product_categories, select#group_order_status').css('width', '95%').select2();
    }else{
        $('select#group_customer_status, select#group_product_categories, select#group_order_status').css('width', '95%').chosen({search_contains:true});
    }
              ...
              beforeSend: function(xhr) {
                ...
                complete: function(xhr) {
                    if($(id == '_billing_country' && 'select#_billing_state').length > 0){
                        if( $().select2 ){
                            $('select#_billing_state').select2();
                        }else{
                            $('select#_billing_state').chosen({search_contains:true});
                        }
                    }                

                    if($(id == '_shipping_country' && 'select#_shipping_state').length > 0){
                        if( $().select2 ){
                            $('select#_shipping_state').select2();
                        }else{
                            $('select#_shipping_state').chosen({search_contains:true}); 
                        }
                    }                
    ...
    if($('#customer_data #_shipping_country').length > 0){
        $('#customer_data').on('click', '#copy-billing-same-as-shipping', function(){
            ...
            if (answer){
                ...
                if( $().select2 ){
                    $('#order_data_column_shipping div.edit_address select').trigger( 'change' );
                }else{
   ...
   if( $('#woocommerce-customer-notes').length > 0 ){
    ...
    $('.open_c_notes').click(function(event) {
...
jQuery(document).ready(function($){
    if( $('#wcrm-customer-activity-tabs').length > 0 ){
        $('#wcrm-customer-activity-tabs a').click(function(event) {
            var id = $(this).attr('href');
            $('#wcrm-customer-activity-tabs li').removeClass('active');
            $(this).closest('li').addClass('active');
            $('.wcrm-customer-activity-panel').hide();
            $(id).show();
            return false;
        });
    }
