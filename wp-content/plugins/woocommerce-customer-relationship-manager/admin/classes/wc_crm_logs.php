...
class WC_Crm_Logs extends WP_List_Table {
  ...
  function get_data(){
        ...
        if( !empty( $_GET['order_id'] ) ){
          ...
          $user_email = get_user_meta( $_GET['userid'], 'email', true );
          if(empty($user_email))
          $user_email = get_user_meta( $_GET['userid'], 'billing_email', true );
        }
        ...
        $orderby = ( ! empty( $_GET['orderby'] ) && in_array($_GET['orderby'], $this->r ) ) ? 'ORDER BY '.$_GET['orderby'] : 'ORDER BY created';
