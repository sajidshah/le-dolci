...
class WC_Crm_Install {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'wc_customer_relationship_manager' ), '2.4.3' );
	}
  ...
  function add_menu() {
    ...
    $accounts_hook = add_submenu_page(
      WC_CRM()->id,
      __('Accounts', 'wc_customer_relationship_manager'),
      __('Accounts', 'wc_customer_relationship_manager'),
      'manage_woocommerce',
  ...
  public function submenu_order( ) {
    ...
    if($post_type == 'wc_crm_accounts'){
      $parent_file = 'wc-customer-relationship-manager';
      $submenu_file = 'edit.php?post_type=wc_crm_accounts';
    }
  ...
  public function action_links( $links ) {
    ...
    $plugin_links['settings'] = sprintf( '<a href="%s">%s</a>', '' . admin_url( 'admin.php?page=wc_crm_settings' ) . '', __( 'Settings', 'wc_customer_relationship_manager' ) );
    
    $plugin_links['support'] = sprintf( '<a href="%s">%s</a>', 'https://actualityextensions.zendesk.com/hc/en-us/categories/200438682-Customer-Relationship-Manager', __( 'Documentation', 'wc_customer_relationship_manager' ) );
  ...
  private function create_tables(){
    ...
    if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}wc_crm_groups_relationships';" ) ) {
        $index_relation = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}wc_crm_groups_relationships WHERE Key_name != 'PRIMARY';" );
        if( !empty($index_relation)){
          $removed = array();
          foreach ($index_relation as $key => $value) {
            if( !in_array($value->Key_name, $removed)){
              $wpdb->query( "ALTER TABLE {$wpdb->prefix}wc_crm_groups_relationships DROP INDEX ".$value->Key_name.";" );
            }
            $removed[] = $value->Key_name;
          }
        }
        //$wpdb->query( "ALTER TABLE {$wpdb->prefix}wc_crm_groups_relationships ADD COLUMN ID bigint(20) NOT NULL AUTO_INCREMENT;" );
    }
