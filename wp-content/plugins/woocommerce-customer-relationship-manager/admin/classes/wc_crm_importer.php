...
class WC_Crm_Importer extends WP_Importer {
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'wc_customer_relationship_manager' ) . '</strong><br />';
			printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'wc_customer_relationship_manager' ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
	...
	{
		$customer_detail = WC_CRM()->customer_detail();
		...
		$keys     = array(
			'default' => __('Select fields to map', 'wc_customer_relationship_manager'),
			'wcrm_custom_meta' => __('Custom meta', 'wc_customer_relationship_manager'),
			'user_role'        => __('Role', 'wc_customer_relationship_manager'),
			'user_login'       => __('User Login', 'wc_customer_relationship_manager'),
			'user_group'       => __('Group', 'wc_customer_relationship_manager'),
			'user_nicename'    => __('Nicename', 'wc_customer_relationship_manager'),
			'display_name'     => __('Display name', 'wc_customer_relationship_manager')
		);
		if($fields){
	...
	?>
		<form action="<?php echo admin_url( 'admin.php?page=wc_crm_import&amp;step=2' ); ?>" method="post">
	          ...
	          <tbody>
	            <tr class="form-field">
	              <th valign="top" scope="row">
	                <label><?php _e( 'Default User Role ', 'wc_customer_relationship_manager' ); ?></label>
	              </th>
	              <td>
	                ...
	                <p class="description"><?php _e( 'Select what the default User Role for the imported customers should be upon import.', 'wc_customer_relationship_manager' ); ?></p>
	              </td>
	            </tr>
	            <tr class="form-field">
	              <th valign="top" scope="row">
	                <label><?php _e( 'Default Status ', 'wc_customer_relationship_manager' ); ?></label>
	              </th>
	              <td>
	                ...
	                <p class="description"><?php _e( 'Select what the default customer status for the imported customers should be upon import.', 'wc_customer_relationship_manager' ); ?></p>
	              </td>
	            </tr>
	            <tr class="form-field">
	              <th valign="top" scope="row">
	                <label><?php _e( 'Skip First Row', 'wc_customer_relationship_manager' ); ?></label>
	              </th>
	              <td>
	                <input type="checkbox" name="skip_first" value="yes" checked="checked">
	                <p class="description"><?php _e( 'Check this box to skip the addition of the first row of the CSV file. You can see the values of the first row below on the left.', 'wc_customer_relationship_manager' ); ?></p>
	              </td>
	            </tr>
	            <tr class="form-field">
	              <th valign="top" scope="row">
	                <label><?php _e( 'Field Mapping', 'wc_customer_relationship_manager' ); ?></label>
	              </th>
	              <td>
	                <?php
			...
			<p class="submit"><input type="submit" class="button button-primary" value="<?php esc_attr_e( 'Import', 'wc_customer_relationship_manager' ); ?>" /></p>
		</form>
	</div>
	<?php
	}
	...
	function options_select(  ) {
		...
		$select .= '<p class="description">'.__('Enter user meta key.','wc_customer_relationship_manager').'</p>';
		$select .= '</div>';
		return $select;
	}
	...
	function parse( $file ) {
		if (($handle = fopen($file, "r")) === FALSE) 
			return new WP_Error( 'Parse_error', __( 'There was an error when reading this file', 'wc_customer_relationship_manager' ) );
		return $handle;
	}
	...
	function greet() {
		echo '<div class="wrap" id="wc-crm-page">';
			echo '<h2>'.__( 'Import ', 'wc_customer_relationship_manager' ).'</h2>';
		  	echo '<div class="narrow">';
			echo '<p>'.__( 'Choose a CSV file to upload, then click Upload file and import.', 'wc_customer_relationship_manager' ).'</p>';
