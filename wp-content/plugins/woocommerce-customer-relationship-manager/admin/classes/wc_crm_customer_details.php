						...
						<div class="meta-box-sortables">
							...
							<?php if( $this->user_id || $this->order_id ){ ?>
								<div class="postbox" id="woocommerce-customer-orders">
									...
									<h3 class="hndle ui-sortable-handle"><span><?php _e( 'Customer Orders', 'wc_customer_relationship_manager' ) ?></span></h3>
									<div class="inside" style="margin:0px; padding:0px;">
								...
								<div class="postbox" id="woocommerce-customer-activity">
									<div title="Click to toggle" class="handlediv"><br></div>
									<h3 class="hndle ui-sortable-handle"><span><?php _e( 'Activity', 'wc_customer_relationship_manager' ) ?></span></h3>
									<div class="inside" style="margin:0px; padding:0px;">
								...
								<div class="postbox" id="wcrm-products-purchased">
									<div title="Click to toggle" class="handlediv"><br></div>
									<h3 class="hndle ui-sortable-handle"><span><?php _e( 'Products Purchased', 'wc_customer_relationship_manager' ) ?></span></h3>
									<div class="inside" style="margin:0px; padding:0px;">
										<?php $this->get_products_purchased(); ?>
									</div>
								</div>
							<?php } ?>
						</div>
	...
	function init_general_fields($user_id = 0)
	{
		...
		$this->general_fields = apply_filters( 'wcrm_admin_general_fields', array(			
			...
			'customer_agent' => array(
				...
				'label' => __( 'Agent', 'wc_customer_relationship_manager' ),
				'value' => $customer_agent,
				'class'   => 'wc-customer-search',
				'type'    => 'text',
				'custom_attributes' => array(
					'data-allow_clear' => true,
					'data-placeholder' => __( 'Select an Agent&hellip;', 'wc_customer_relationship_manager' ),
			...
			'date_of_birth' => array(
				...
				'value' => get_user_meta( $user_id, 'date_of_birth', true ),
				'custom_attributes' => array(
			...
			'customer_mobile' => array(
				...
				'value' => get_user_meta( $user_id, 'mobile', true )
			),
			'customer_fax' => array(
				'meta_key'  => 'fax',
				'label' => __( 'Fax', 'wc_customer_relationship_manager' ),
				'value' => get_user_meta( $user_id, 'fax', true )
			),
			'customer_site' => array(
				'meta_key'  => 'url',
			...
			'customer_lead_status' => array(
				...
				'value' => get_user_meta( $user_id, 'lead_status', true ),
				'class' => 'wc-enhanced-select',
				'type'    => 'select',
				'custom_attributes' => array(
					'data-allow_clear' => true,
			...
			'customer_industry' => array(
				...
				'value' => get_user_meta( $user_id, 'industry', true ),
				'class' => 'wc-enhanced-select',
				'type'    => 'select',
				'custom_attributes' => array(
					'data-allow_clear' => true,
...
/**
	...
	function	get_formatted_billing_address($map = false) {
		if ( ! $this->formatted_billing_address ) {
			...
			foreach ( $this->billing_fields as $key => $field ) {
				if($this->user_id){
					$address[$key] = get_user_meta( $this->user_id, 'billing_'.$key, true );
				}
				elseif($this->order_id){
					$name_var = 'billing_'.$key;
	...
	function get_formatted_shipping_address() {
		if ( ! $this->formatted_shipping_address ) {
			...
			foreach ( $this->shipping_fields as $key => $field ) {
				if($this->user_id){
					$address[$key] = get_user_meta( $this->user_id, 'shipping_'.$key, true );
				}
				elseif($this->order_id){
	...
	function get_products_purchased(){
		require_once( 'wc_crm_customer_product_list.php');
		$wc_crm_product_list = new WC_Crm_Customer_Product_List($this->user_data);
