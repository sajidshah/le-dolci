...
class WC_Crm_Customers_Table extends WP_List_Table {
  ...
  function get_columns() {
    ...
    if( class_exists( 'WC_Subscriptions' ) && $wc_subscriptions == 'yes') {
      $columns['wc_subscriber'] = __( 'Subscribed', 'wc_customer_relationship_manager' );
    }

    $groups_wc = get_option('wc_crm_show_groups_wc_column');
    if( class_exists( 'Groups_WordPress' ) && class_exists( 'Groups_WS' ) && $groups_wc == 'yes' ) {
      $columns['wc_group'] = __( 'Group', 'wc_customer_relationship_manager' );
    }
  ...
  function column_wc_subscriber( $item ) {
    ...
    if( $item['user_id'] && !empty($item['user_id']) ){
      if ( WC_Subscriptions_Manager::user_has_subscription( $item['user_id'], '', 'active' ) ) {
        ...
        return "<span class='enrolled-yes'></span>";
  ...
  function get_views(){
    ...
    if( !empty($woocommerce_crm_user_roles)){
      $user_roles = "WHERE capabilities LIKE '%". implode("%' OR capabilities LIKE '%", $woocommerce_crm_user_roles) . "%'";
    }
    if( !empty($user_roles) ){
      $sql = "SELECT status, count(status) as count FROM (SELECT * FROM {$wpdb->prefix}wc_crm_customer_list $user_roles) as wc_crm_customer_list GROUP BY status";
    }else{
      $sql = "SELECT status, count(status) as count FROM {$wpdb->prefix}wc_crm_customer_list GROUP BY status";
    }
    ...
    if($statuses){
     ...
     $all_url = 'admin.php?page=wc-customer-relationship-manager';
     $views['all'] = "<a href='{$all_url }' {$class} >All <span class='count'>({$all})</span></a>";

     if($statuses){
      foreach ($statuses as $st) {
        $url   = add_query_arg( array('_customer_status' => $st->status), 'admin.php?page=wc-customer-relationship-manager' );
