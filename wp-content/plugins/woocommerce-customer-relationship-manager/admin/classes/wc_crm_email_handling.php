...
class WC_Crm_Email_Handling {
	...
	public static function display_form() {
		...
		if ( isset( $_REQUEST['user_id'] ) && !is_array( $_REQUEST['user_id'] ) ) { // if we have only one element, make it as array
			...
			array_push( $recipients, get_user_meta( $user_id, 'user_email', true ) );
			array_push( $phones, get_user_meta( $user_id, 'billing_phone', true ) );
		} else if ( isset( $_REQUEST['user_id'] ) && is_array( $_REQUEST['user_id'] ) ) {
			$user_ids = $_REQUEST['user_id'];
			foreach ($user_ids as $id) {
				array_push( $recipients, get_user_meta( $id, 'user_email', true ) );
				array_push( $phones, get_user_meta( $id, 'billing_phone', true ) );
			}
		}
