    ...
    return;
		...
		if( !empty($action) ){
			...
			if(array_key_exists($action, $statuses) ){
				...
				if( !isset($_REQUEST['user_id']) || empty($_REQUEST['user_id']) ) return;
				$user_ids = $_REQUEST['user_id'];
				wc_crm_change_customer_status($action, $user_ids);

			}else if( strstr( $action, 'crm_add_to_group_') ) {

				if( (!isset($_REQUEST['user_id']) || empty($_REQUEST['user_id']) ) && ( !isset($_REQUEST['order_id']) || empty($_REQUEST['order_id']) ) ) return;
					$filer = '';
...
function wc_customer_relationship_manager_render_list_page() {
	...
	?>
	<form method="<?php echo empty($page_action) ? 'get' : 'post'; ?>" id="wc_crm_customers_form" action="admin.php?page=wc-customer-relationship-manager<?php echo $group; ?>">
...
function wc_crm_page_title($request){
	...
	if( ( isset($_REQUEST['group']) && !empty( $_REQUEST['group'] ) ) ){
		...
		echo '<h2>' . __( 'Customers group ', 'wc_customer_relationship_manager' ) . $group_name .' <a href="'.admin_url().'admin.php?page=wc_new_customer" class="add-new-h2">Add Customer</a>'.$s.'</h2>';
		...
		echo '<h2>' . __( 'Products ', 'wc_customer_relationship_manager' ) . '</h2>';
			...
			echo '<h2>' . __( 'Customers', 'wc_customer_relationship_manager' ) . ' <a href="'.admin_url().'admin.php?page=wc_new_customer" class="add-new-h2">Add Customer</a>'.$s.'</h2>';
	}
...
function wc_crm_page_messages($request){
	...
	if ( isset($request['update']) ) {
			...
			switch ( $request['update'] ) {
				case "sent_email":
					$messages[] =  __( 'Email sent.', 'wc_customer_relationship_manager' );
					break;
				case "save_phone_call":
					$messages[] =  __( 'Phone call saved.', 'wc_customer_relationship_manager' );
					break;
			}
	}
...
function wc_crm_update_user_groups($group_ids = array(), $customers_email = ''){
	...
	if( empty($customers_email) ) return false;
	global $wpdb;
