<?php
/**
 * Swiss Design Group Customizer functionality
 *
 * @package WordPress
 ...
 */
function SDG_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'SDG_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', HB_DOMAIN_TXT ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => SDG_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', HB_DOMAIN_TXT ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', HB_DOMAIN_TXT ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', HB_DOMAIN_TXT ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', HB_DOMAIN_TXT ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', HB_DOMAIN_TXT );
}
add_action( 'customize_register', 'SDG_customize_register', 11 );

/**
 * Register color schemes for Swiss Design Group.
 *
 * Can be filtered with {@see 'SDG_color_schemes'}.
 *
 ...
 */
function SDG_get_color_schemes() {
	return apply_filters( 'SDG_color_schemes', array(
		'default' => array(
...
if ( ! function_exists( 'SDG_get_color_scheme' ) ) :
/**
 * Get the current Swiss Design Group color scheme.
 *
 * @since Swiss Design Group 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function SDG_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = SDG_get_color_schemes();
...
endif; // SDG_get_color_scheme

if ( ! function_exists( 'SDG_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for Swiss Design Group.
 *
 * @since Swiss Design Group 1.0
 *
 * @return array Array of color schemes.
 */
function SDG_get_color_scheme_choices() {
	$color_schemes                = SDG_get_color_schemes();
...
endif; // SDG_get_color_scheme_choices

if ( ! function_exists( 'SDG_sanitize_color_scheme' ) ) :
/**
 ...
 */
function SDG_sanitize_color_scheme( $value ) {
	$color_schemes = SDG_get_color_scheme_choices();
...
endif; // SDG_sanitize_color_scheme
...
function SDG_color_scheme_css() {
	...
	$color_textcolor_rgb         = SDG_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = SDG_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = SDG_get_color_scheme_css( $colors );

	wp_add_inline_style( 'SDG-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'SDG_color_scheme_css' );
 ...
 * @since Swiss Design Group 1.0
 */
function SDG_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', SDG_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'SDG_customize_control_js' );
 ...
 * @since Swiss Design Group 1.0
 */
function SDG_customize_preview_js() {
	wp_enqueue_script( 'SDG-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'SDG_customize_preview_js' );
 ...
 */
function SDG_get_color_scheme_css( $colors ) {
 ...
 * @since Swiss Design Group 1.0
 */
function SDG_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-SDG-color-scheme">
		<?php echo SDG_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'SDG_color_scheme_css_template' );
